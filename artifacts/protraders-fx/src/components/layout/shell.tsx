...
import { Activity, BarChart3, Bot, Camera, ChevronRight, CircleAlert, LayoutDashboard, Settings2 } from "lucide-react"
...
export function Shell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-[100dvh] bg-background text-foreground">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col bg-sidebar text-sidebar-foreground md:flex">
      ...
      <div className="mt-auto border-t border-sidebar-border p-5"><div className="text-[10px] uppercase tracking-[.2em] text-sidebar-foreground/40">Execution mode</div><div className="mt-2 flex items-center gap-2 text-sm"><span className="h-2 w-2 rounded-full bg-sidebar-primary" /> Controlled / review-first</div></div>
    </aside>
