...
router.post("/bots/:id/run-once", async (req, res) => { const auth = await authenticated(req, res); if (!auth) return; const [bot] = await db.select().from(bots).where(and(eq(bots.id, req.params.id), eq(bots.ownerKey, auth.key))); if (!bot) return fail(res, 404, "Bot not found"); if (bot.status === "archived") return fail(res, 409, "Archived bot cannot run"); const strategy = dryRunStrategySchema.safeParse(bot.config); if (!strategy.success) return fail(res, 409, "Bot strategy is invalid"); const [run] = await db.insert(botRuns).values({ ownerKey: auth.key, botId: bot.id, mode: "dry_run", status: "running" }).returning(); try { const data = await publicDeriv({ ticks_history: bot.symbol, style: "candles", granularity: 300, count: 60, end: "latest" }); const candles = data.candles.map((c: any) => ({ epoch: Number(c.epoch), close: Number(c.close) })).filter((c: any) => Number.isFinite(c.epoch) && Number.isFinite(c.close)); if (candles.length < 30) throw new Error("Insufficient candle data"); const indicator = metrics(candles); const signal = strategy.data.indicator === "ema" ? indicator.trend === "up" ? "CALL" : indicator.trend === "down" ? "PUT" : null : strategy.data.indicator === "rsi" ? indicator.rsi14 < 30 ? "CALL" : indicator.rsi14 > 70 ? "PUT" : null : indicator.macd > indicator.macdSignal ? "CALL" : indicator.macd < indicator.macdSignal ? "PUT" : null; const allowed = signal && (strategy.data.direction === "BOTH" || strategy.data.direction === signal); const recommendation = { action: allowed ? `review-${signal.toLowerCase()}` : "no-action", dryRun: true, exactInputs: { symbol: bot.symbol, indicator: strategy.data.indicator, direction: strategy.data.direction, stake: strategy.data.stake, duration: strategy.data.duration, riskCap: strategy.data.riskCap, execution: "dry_run" }, simulationMetadata: ["stake", "duration", "riskCap"], indicators: indicator, disclaimer: "Dry-run only: no order was placed. Stake, duration, and risk cap are simulation metadata, not execution instructions." }; const [completed] = await db.update(botRuns).set({ status: "completed", result: recommendation, completedAt: new Date() }).where(eq(botRuns.id, run.id)).returning(); await db.insert(botEvents).values({ ownerKey: auth.key, botId: bot.id, runId: run.id, type: "run.evaluated", payload: recommendation }); return res.json(completed); } catch (e) { const message = e instanceof Error ? e.message : "evaluation failed"; await db.update(botRuns).set({ status: "failed", result: { error: message }, completedAt: new Date() }).where(eq(botRuns.id, run.id)); await db.insert(recoveryIncidents).values({ ownerKey: auth.key, botId: bot.id, title: "Bot dry-run evaluation failed", severity: "medium", facts: { runId: run.id, error: message } }); return fail(res, 502, "Bot evaluation unavailable", message); } });
