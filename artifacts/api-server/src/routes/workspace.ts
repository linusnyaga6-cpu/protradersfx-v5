...
type DryRunStrategy = z.infer<typeof dryRunStrategySchema>;
...
const builtIns = [
  ...
  { id: "protraders-bot-2-recovery-monitor", botNumber: 2, name: "Recovery Bot", source: "exwager.com", sourceUrl: "https://exwager.com", description: "Recovery bot from exwager.com. Monitor-only.", strategy: { indicator: "rsi", direction: "BOTH", mode: "recovery_guard", stake: 1, duration: 5, riskCap: 5, notes: "Monitor and pause for review. Never increase stake, retry an order, or place a trade.", execution: "dry_run" } },
...
router.post("/templates", async (req, res) => { const auth = await authenticated(req, res); if (!auth) return; const name = string(req.body?.name), description = string(req.body?.description, 1000), strategy = dryRunStrategySchema.safeParse(req.body?.strategy); if (!name || !description || !strategy.success) return fail(res, 400, "Invalid template"); const normalized = normalizeDryRunStrategy(strategy.data); if (!hasCompatibleRiskCap(normalized)) return fail(res, 400, "Template stake and runs exceed its risk cap"); const [item] = await db.insert(botTemplates).values({ ownerKey: auth.key, name, description, strategy: normalized }).returning(); return res.status(201).json(item); });
router.patch("/templates/:id", async (req, res) => { const auth = await authenticated(req, res); if (!auth) return; const name = req.body?.name === undefined ? undefined : string(req.body.name), description = req.body?.description === undefined ? undefined : string(req.body.description, 1000); const strategy = req.body?.strategy === undefined ? undefined : dryRunStrategySchema.safeParse(req.body.strategy); if ((req.body?.name !== undefined && !name) || (req.body?.description !== undefined && !description) || (strategy && !strategy.success)) return fail(res, 400, "Invalid template"); const normalized = strategy?.success ? normalizeDryRunStrategy(strategy.data) : undefined; if (normalized && !hasCompatibleRiskCap(normalized)) return fail(res, 400, "Template stake and runs exceed its risk cap"); const [item] = await db.update(botTemplates).set({ ...(name ? { name } : {}), ...(description ? { description } : {}), ...(normalized ? { strategy: normalized } : {}), updatedAt: new Date() }).where(and(eq(botTemplates.id, req.params.id), eq(botTemplates.ownerKey, auth.key))).returning(); return item ? res.json(item) : fail(res, 404, "Template not found"); });
...
router.post("/bots", async (req, res) => { const auth = await authenticated(req, res); if (!auth) return; const name = string(req.body?.name), symbol = string(req.body?.symbol, 30); const parsed = dryRunStrategySchema.safeParse(req.body?.config); if (!name || !symbol || !isVolatilitySymbol(symbol) || !parsed.success) return fail(res, 400, "Invalid bot dry-run strategy"); const normalized = normalizeDryRunStrategy(parsed.data); if (!hasCompatibleRiskCap(normalized)) return fail(res, 400, "Bot stake and runs exceed its risk cap"); let templateId: string | null = null; if (req.body?.templateId !== undefined && req.body.templateId !== null && req.body.templateId !== "") { if (!uuidSchema.safeParse(req.body.templateId).success) return fail(res, 400, "Invalid template ID"); const [template] = await db.select({ id: botTemplates.id }).from(botTemplates).where(and(eq(botTemplates.id, req.body.templateId), eq(botTemplates.ownerKey, auth.key), isNull(botTemplates.archivedAt))).limit(1); if (!template) return fail(res, 404, "Template not found"); templateId = template.id; } const [bot] = await db.insert(bots).values({ ownerKey: auth.key, name, symbol, config: normalized, templateId }).returning(); return res.status(201).json(bot); });
router.patch("/bots/:id", async (req, res) => { const auth = await authenticated(req, res); if (!auth) return; const name = req.body?.name === undefined ? undefined : string(req.body.name); const symbol = req.body?.symbol === undefined ? undefined : string(req.body.symbol, 30); const config = req.body?.config === undefined ? undefined : dryRunStrategySchema.safeParse(req.body.config); if ((req.body?.name !== undefined && !name) || (req.body?.symbol !== undefined && (!symbol || !isVolatilitySymbol(symbol))) || (config && !config.success)) return fail(res, 400, "Invalid bot update"); const normalized = config?.success ? normalizeDryRunStrategy(config.data) : undefined; if (normalized && !hasCompatibleRiskCap(normalized)) return fail(res, 400, "Bot stake and runs exceed its risk cap"); const [bot] = await db.update(bots).set({ ...(name ? { name } : {}), ...(symbol ? { symbol } : {}), ...(normalized ? { config: normalized } : {}), updatedAt: new Date() }).where(and(eq(bots.id, req.params.id), eq(bots.ownerKey, auth.key))).returning(); return bot ? res.json(bot) : fail(res, 404, "Bot not found"); });
