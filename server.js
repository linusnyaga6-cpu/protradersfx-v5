 ...
 * Do NOT write files to /var/task.
 *
 * The website files are in the repository root.
 */
...
/* -------------------------------------------------------
   SIMPLE SERVERLESS-SAFE ANALYTICS
------------------------------------------------------- */

/*
 * IMPORTANT:
 * We intentionally do not use fs/writeData here.
 *
 * Vercel serverless functions cannot be treated like a
 * normal VPS filesystem.
 *
 * These counters are temporary/in-memory only.
 * Permanent analytics should later use a database or
 * analytics service.
 */
...
app.get('/api/analytics', (req, res) => {
  ...
  res.json({
    ...
    oauthSuccesses:
      events.filter(
        e =>
          e.type === 'oauth_login_success' ||
          e.type === 'oauth_signup_success'
      ).length,

    fundedAccounts: null,

    note:
      'Funded-account status must be confirmed in Deriv Partner Hub and is not fabricated here.'
